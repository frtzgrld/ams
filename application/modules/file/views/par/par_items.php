...
<div id="trancon-close-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
        <form id="par_details_save" action="" method="post" class="form-horizontal" role="form">
        <input type="hidden" name="par_id" id="par_id" value="<?php echo $par_id; ?>">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <table id="datatabletran" class="table table-striped table-hover dt-responsive nowrap">
                                <thead>
                                    <tr>
                                        <th width="10%"></th>
                                        <th width="15%">Property No</th>
            ...
            <input type="hidden" name="par_id" id="par_id" value="<?php echo $par_id; ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2" align="right">
                        <label class="control-label-right" align="right">PAR No.<span class="text-danger">*</span></label>
                    </div>
                    
                    <div class="col-sm-3" for="createdby" align="left">
                        <input type="text" class="form-control" name="par_no" id="par_no" value="<?php echo $par_no; ?>">
                    </div>
                    <!-- <div class="col-md-9 col-md-offset-9 error-message"></div> -->
                </div>
        ...
        <div class="row">
            <div class="col-sm-6">
                <div class="m-b-30">
                    ...
                    <button type="button" class="btn btn-primary waves-effect waves-light" data-toggle="modal" data-target="#trancon-close-modal">Transfer Items</button>
                </div>
            </div>
        </div>
<?php }?>
        ...
        <!-- <script type="text/javascript">
                        $(document).ready(function(){
                            ...
                            var par_id = <?php echo $par_id; ?>;
                            $('#button_placement').html(
                                    '<button class="btn btn-block btn-info" onclick="jQuery(popup_window(\''+base_url+'file/par/print_par/' + par_id + '\'));"><i class="fa fa-print"></i> PRINT PROPERTY ACKNOWLEDGEMENT RECEIPT</button>'
                                );
                        })
                    </script> -->
                   
        <button class='btn btn-icon waves-effect waves-light btn-info m-b-5 btn-sm' onclick='return print_report(<?php echo $par_id; ?>)'> <i class='ti ti-printer'></i></button>
          ...
          <tbod>
              <?php 
           if(!empty($par_items_list))
            {
                // echo print_r($items,true);
                foreach ($par_items_list as $key) 
                :?>
                <tr>
                    <td>1</td>
                    <td>unit</td>
                    ...
                    <td> <?php if($key['id'] == null) { echo '<button class="btn btn-primary waves-effect waves-light" onclick="deleteItem('.$key['id'].')" >Add</button>'; } 
                    // else { echo '<button class="btn btn-primary waves-effect waves-light" onclick="deleteItem('.$key['id'].')" >Delete</button>'; }
                    ?> </td>
                </tr>
                <?php endforeach; 
            }
                ...
                <div class="row">
                    <div class="form-group m-b-0">
                        <div class="col-sm-offset-7 col-sm-4">
                         ...
                         <button type="button" onclick="updatePAR(<?php echo $par_id; ?>)" class="btn btn-primary waves-effect w-md waves-light m-b-5">Save</button>
<!-- 
...
$(document).ready(function () {
        ...
        $('#datatable').dataTable({
            ...
            "serverSide": true,
            "ajax": base_url + 'file/par/datatable_par_items',
            "order": [[ 0, "asc" ]],
    ...
    {
        $.ajax({
            url: base_url + 'file/par/save_items',
            method: 'POST',
            cache: false,
            dataType: 'json',
            data: {prop_id: id, par_id: <?php echo $par_id; ?>},
            async: true,
    ...
    {
        $.ajax({
            url: base_url + 'file/par/delete_items',
            method: 'POST',
            cache: false,
            dataType: 'json',
            data: {prop_id: id, par_id: <?php echo $par_id; ?>},
...
function updatePAR( id )
    {
        ...
        $.ajax({
            url: base_url + 'file/par/par_header_update',
            method: 'POST',
            cache: false,
            dataType: 'json',
...
$(document).ready(function () {
        ...
        $('#datatabletran').dataTable({
            "processing": true,
            "language": {
                "processing": '<img src="'+base_url+'assets/images/loaders/search_loader_1.gif" width="120px"/> Loading. Waiting for response...'
            },
            "serverSide": true,
            "ajax": base_url + 'file/par/datatable_par_items_trans',
            "order": [[ 0, "asc" ]],
            "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) 
            {
                var chckbox = '<button class="btn btn-xs btn-block" type="button" onclick="getTranItem(' + aData[0] +')">SELECT</button>';
                // <input type="radio" name="pr_no[]" id="pr_no['+aData[2]+']" value="' + aData[2] + '">';
    ...
    var print_report = function(id){
                window.open(base_url + 'file/par/view_par/' + id, '_blank');
            }
    </script>