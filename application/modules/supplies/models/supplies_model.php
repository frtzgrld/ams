...
LEFT JOIN items ON items.id = ris_items.`REQ_DESCRIPTION`',
    ...
    {
        $where = $item ? 'WHERE supplies.items = '.$item : null;

        $tableset = array(
                'object'    => 'distribution',
                'index'     => 'distribution.ID',
                'join'      => 'LEFT JOIN supplies ON supplies.ID = distribution.SUPPLIESID
				left join offices on offices.id = distribution.ASSIGNED_TO_OFFICE',
                'where'     => $where,
                'group'     => null,
                'columns'   => array(
                        'offices.DESCRIPTION',
                        'distribution.ASSIGNED_QTY',
                        'distribution.ASSIGNED_DATE',
                        'distribution.ASSIGNED_by',
                        // 'ris.ID',
                    ),
            );
